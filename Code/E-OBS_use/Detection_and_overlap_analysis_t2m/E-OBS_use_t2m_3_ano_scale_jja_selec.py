...
nc_in_path=os.path.join(datadir,"E-OBS","t2m",f"{the_variable}_ens_mean_0.1deg_reg_v26.0e.nc")
...
nc_file_anomaly=os.path.join(datadir,"E-OBS","t2m",the_variable+"_daily_avg_"+str(year_beg)+"_"+str(year_end)+"_smoothed.nc")  #path to the netCDF file
...
nc_out_name = os.path.join(datadir,"E-OBS","t2m",the_variable+"_anomaly_JJA_"+str(year_beg)+"_"+str(year_end)+"_scaled_"+str(threshold_value)+"th.nc")
...
nc_out_not_scaled_path = os.path.join(datadir,"E-OBS","t2m",f"{the_variable}_anomaly_JJA_{year_beg}_{year_end}.nc")#path to the output netCDF file
...
#-------------------------------------
#%%
f_threshold_name = os.path.join(datadir,"E-OBS","t2m","distrib_"+the_variable+"_ano_"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th_threshold_15days.nc")
...
for i in tqdm(range(year_end-year_beg+1)) :
    ...
    for j in range(92):#92 days of JJA for each year i
        t2m[92*i+j,:,:]=ma.array(f.variables[the_variable][idx_start_year[i]+152-bis_year_flag+j,:,:]-T_mean[j,:,:])
        t2m_not_scaled[92*i+j,:,:]=ma.array(f.variables[the_variable][idx_start_year[i]+152-bis_year_flag+j,:,:]-T_mean[j,:,:])
