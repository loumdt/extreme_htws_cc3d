...
#%%
nc_file_in = os.path.join(datadir , "E-OBS" ,"t2m", "Detection_Canicule" , "detected_heatwaves_"+the_variable+"_anomaly_JJA_"+str(year_beg)+"_"+str(year_end)+"_threshold_"+str(threshold_value)+"th_"+str(nb_days)+"days.nc")
...
for emdat_event in tqdm(df_emdat.index.values[:]) :
    if df_emdat.loc[emdat_event,'Dis No'] not in ignored_events :
        ...
        f_mask=nc.Dataset(os.path.join(datadir, "E-OBS","Mask","Mask_"+country_dict[country]+"_E-OBS_0.1deg.nc"),mode='r')
...
output_dir = os.path.join("Output","E-OBS",the_variable,the_variable+"_"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th_threshold_"+str(nb_days)+"days")
pathlib.Path(output_dir).mkdir(parents=True,exist_ok=True)
with open(os.path.join(output_dir,"emdat_undetected_heatwaves_E-OBS_"+the_variable+"_"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th_threshold_"+str(nb_days)+"days.txt"), 'w') as output :
    for row in undetected_heatwaves:
        output.write(str(row) + '\n')
        
with open(os.path.join(output_dir,"emdat_detected_heatwaves_E-OBS_"+the_variable+"_"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th_threshold_"+str(nb_days)+"days.txt"), 'w') as output :
