...
'pseudo_Russo_area_pop_PL': np.logspace(start=np.log10(9), stop=np.log10(121629357900), num=30)} #bins of the histogram
...
'multi_index_pop_mean': 9, 'pseudo_Russo_area_pop_PL': 7} #window of the savitzky-golay filter for the histogram smoothing
...
chosen_meteo = 'pseudo_Russo_area_pop_PL'
chosen_impact = 'Total_Deaths'
coeff_PL_list=range(2,5)
threshold_PL_list=[0]*103
threshold_PL_list[0:3]=10,50,70
threshold_PL_list[3:]=range(100,10100,100)
file_txt=open("/home/theom/Bureau/Ubuntu_SSD/PFE/Code/Van_der_Wiel_graphs/try_PL/criteria_distrib_correlations_try_PL.txt",'w')

emdat_htw = np.zeros((len(threshold_PL_list),len(coeff_PL_list),len(stefanon_htw),3)) #impact,meteo,htw (htw_impact,htw_meteo,htw_number) in order to evalujate correlations at the end

personR_table=np.zeros((len(threshold_PL_list),len(coeff_PL_list)))

for threshold_PL in threshold_PL_list :
    nb_coeff = 0
    for coeff_PL in  coeff_PL_list :
        #threshold_PL = 2500
        #coeff_PL = 2
        print('threshold_PL',threshold_PL, 'coeff_PL', coeff_PL)
        ...
        for i in range(len(heatwaves_idx_2)) : #each heatwave has a number, ranging from 0 to 305 in my case
            ...
            if i not in not_computed_htw and i not in careful_htw:
                ...
                meteo_list=np.append(meteo_list,np.sum((cell_area_3d*russo_idx_map*pop)*coeff_PL*(pop>threshold_PL)*(temp>0))+np.sum(cell_area_3d*russo_idx_map*pop*(pop<=threshold_PL)*(temp>0)))
        ...
        plt.savefig("/home/theom/Bureau/Ubuntu_SSD/PFE/Code/Van_der_Wiel_graphs/try_PL/Heatwaves_distrib_"+chosen_impact+"_"+chosen_meteo+"pow"+str(coeff_PL)+"thresh"+str(threshold_PL)+".png")
        #plt.show()
        #break
        ...
        personR_table[nb_threshold,nb_coeff] = rvalue
        ...
        file_txt.write("coeff "+str(coeff_PL)+" thresh "+str(threshold_PL)+' : pearson R :'+str(stats.pearsonr(np.log10(idx_scatt),np.log10(impact_list)))+" "+'kendall tau :'+str(stats.kendalltau(np.log10(idx_scatt),np.log10(impact_list)))+" "+'spearman R :'+str(stats.spearmanr(np.log10(idx_scatt),np.log10(impact_list)))+ '\n')
        plt.plot([X0,X1],[Y0,Y1], 'k-')
        plt.annotate('R='+str(round(rvalue,5)),((X0+X1)/2,(Y0+Y1/2)))
        plt.grid()
        plt.ylabel('Log(Total_Deaths)')#+units_dico[meteo_criteria[j]])
        plt.xlabel('Log(pRusso_area_pop_PL) pow'+str(coeff_PL)+'thresh'+str(threshold_PL))

        plt.savefig("/home/theom/Bureau/Ubuntu_SSD/PFE/Code/Van_der_Wiel_graphs/try_PL/correlations_try_PL_pow"+str(coeff_PL)+"thresh"+str(threshold_PL)+".png")
        fig.clear()
        nb_coeff += 1
    #break
    nb_threshold += 1
...
print('max pearson :',np.max(personR_table), 'min pearson :',np.min(personR_table), 'argmax pearson :',np.argmax(personR_table))
...
plt.savefig("/home/theom/Bureau/Ubuntu_SSD/PFE/Code/Van_der_Wiel_graphs/try_PL/Heatwaves_distrib_correlations_try_PL.png")
