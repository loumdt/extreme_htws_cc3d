...
#%%
pathlib.Path("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days","figs","distrib"+"_count_all_impacts"*count_all_impacts).mkdir(parents=True, exist_ok=True) #create output directory and parent directories if necessary
...
for chosen_impact in impact_criteria :
    ...
    for chosen_meteo in meteo_criteria :
        ...
        #plt.tight_layout()
        plt.savefig(os.path.join("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days","figs","distrib"+"_count_all_impacts"*count_all_impacts,f"distrib_{chosen_impact}_{chosen_meteo}_{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days"+"_count_all_impacts"*count_all_impacts+".png"))
        fig.clear()
        #plt.show()
#%%