...
#%%
count_all_impacts = True #True : count all affected countries according to EM-DAT ; False : #count only visibly affected countries according to E-OBS
print("count_all_impacts :",count_all_impacts)
#%%
df_htw = pd.read_excel(os.path.join("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days",f"df_htw_{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days"+"_count_all_impacts"*count_all_impacts+".xlsx"),header=0,index_col=0)
#%%
...
pathlib.Path("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days","figs","roc_curve").mkdir(parents=True, exist_ok=True) #create output directory and parent directories if necessary
#%%
for chosen_impact in tqdm(impact_criteria) :
    for chosen_meteo in meteo_criteria :
        ...
        if chosen_impact==impact_criteria[0] : #save this fig only once since it does not depend on the impact criterion
            ...
            plt.legend()
            plt.savefig(os.path.join("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days","figs","roc_curve",f"roc_curve_{chosen_meteo}.png"))
            plt.close()
#%%
df_scores.to_excel(os.path.join("Output","E-OBS",the_variable,f"{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days",f"df_scores_{the_variable}_{year_beg}_{year_end}_{threshold_value}th_threshold_{nb_days}days"+"_count_all_impacts"*count_all_impacts+".xlsx"))