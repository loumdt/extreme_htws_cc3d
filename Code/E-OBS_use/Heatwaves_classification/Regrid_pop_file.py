...
pop_table = np.ma.array(np.zeros((21,465,705)),mask=[mask_all]*21)
...
for year in range(2000,2021) :
    print(year)
    file_in = "/home/theom/Bureau/Ubuntu_SSD/PFE/Data/Pop/WorldPop/ppp_"+str(year)+"_reproj_Aggregated.nc"
    ...
    nc_file_out=nc.Dataset("/home/theom/Bureau/Ubuntu_SSD/PFE/Data/Pop/WorldPop/World_pop_"+str(year)+"_e-obs_grid.nc",mode='w',format='NETCDF4_CLASSIC') #path to the output netCDF file
    ...
    nc_file_out.title="WorldPop population density for year " + str(year)
    ...
    pop_table[year-2000,:,:]=pop_density[:]
    ...
    # CS1 = ax.pcolormesh(lon_grid,lat_grid,pop_density,cmap='seismic',transform=proj_pc,vmin=0,vmax=1.5e)
...
nc_file_out=nc.Dataset("/home/theom/Bureau/Ubuntu_SSD/PFE/Data/Pop/WorldPop/World_pop_all_2000-2020_e-obs_grid.nc",mode='w',format='NETCDF4_CLASSIC') #path to the output netCDF file

#Define netCDF output file :
print('nc_file_out',nc_file_out)
nc_file_out.createDimension('lat', 465)    # latitude axis
nc_file_out.createDimension('lon', 705)    # longitude axis
nc_file_out.createDimension('time', 21)    # time axis

nc_file_out.title="WorldPop population density for all years from 2000 to 2020"
nc_file_out.subtitle="adapted for E-OBS 0.1° resolution grid"
...
time_out.units = 'years from 2000'
time_out.long_name = 'time in years from 2000'
