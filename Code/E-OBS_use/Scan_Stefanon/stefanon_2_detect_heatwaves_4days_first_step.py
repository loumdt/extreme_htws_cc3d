"""Delete the temperature data if it is not strictly positive for at least 4 consecutive days. 
...
try :
    nc_file = os.path.join(os.environ["DATADIR"] , "E-OBS" , "0.1deg" , the_variable+'_anomaly_JJA_'+str(year_beg)+'_'+str(year_end)+'_scaled_'+str(threshold_value)+'th.nc')
except :
    nc_file = 'Data/E-OBS/0.1deg/'+the_variable+'_anomaly_JJA_'+str(year_beg)+'_'+str(year_end)+'_scaled_'+str(threshold_value)+'th.nc'
#%%    
...
#-------------------
try :
    nc_out_name = os.path.join(os.environ["DATADIR"] , "E-OBS" , "Detection_Canicule" , "potential_heatwaves_"+the_variable+"_4days_before_scan"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th.nc")
except :
    nc_out_name = "Data/E-OBS/Detection_Canicule/potential_heatwaves_"+the_variable+"_4days_before_scan"+str(year_beg)+"_"+str(year_end)+"_"+str(threshold_value)+"th.nc"#path to the output netCDF file
...
nc_file_out.subtitle="values are masked where and when not exceeding "+str(threshold_value)+"th temperature anomaly threshold for 4 consecutive days or more. Created with detect_heatwaves_4days_first_step.py on "+ datetime.today().strftime("%d/%m/%y")
...
#-------------------
try :
    nc_file_mask = os.path.join(os.environ["DATADIR"] , "E-OBS" , "Mask" , "Mask_Europe_E-OBS_0.1deg.nc")
except :
...
for year in tqdm(range((year_end-year_beg+1))) :
    stack_temp=ma.array(-9999*np.ones((92,len(lat_in),len(lon_in))),mask=[mask_all]*92) #create a 3D variable that will hold the temperature anomalies when and where there are heatwaves
    stack_where=ma.array(np.zeros((len(lat_in),len(lon_in))),mask=mask_all)
    for day in range(92):
        temp_masked_filled=ma.filled(f.variables['temp'][year*92+day,:,:],fill_value=-9999)
        ...
        if day>=3 :
            ...
            #    stack_temp[day-3:day+1,j,k]=f.variables['temp'][year*92+day-3:year*92+day+1,j,k]
            stack_temp[day-3:day+1,:,:] = stack_temp[day-3:day+1,:,:]*(stack_temp[day-3:day+1,:,:]!=-9999)+f.variables['temp'][year*92+day-3:year*92+day+1,:,:]*((stack_where>=4)*stack_temp[day-3:day+1,:,:]==-9999)+(-9999*((stack_where<4)*(stack_temp[day-3:day+1,:,:]==-9999))) #record the last four days for the corresponding scanning window
