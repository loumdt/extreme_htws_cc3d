...
'Temp_sum_pop_NL': 9, 'Pseudo_Russo_pop_NL': 7, 'Pop_unique': 9, 'Multi_index_Russo':9, 'Multi_index_temp':9, 'Multi_index_Russo_NL':9, 'Multi_index_temp_NL':9} #window of the savitzky-golay filter for the histogram smoothing

clrs_dico = {"Total_Deaths": clrs.LogNorm(vmin=1, vmax=72210), "Total_Affected": clrs.Normalize(vmin=0, vmax=500), "Total_Damages": clrs.LogNorm(vmin=1, vmax=12120000) , "Impact_fct": clrs.LogNorm(vmin=1e4, vmax=157e6)} #colormap depending on the selected criterion
...
impact_criteria = ['Total_Deaths', 'Impact_fct']
...
file_txt=open("D:/Ubuntu/PFE/JUICCE/Output/Van_der_Wiel_graphs/"+the_variable+"/criteria_distrib_correlations_"+the_variable+".txt",'w')
file_txt.write('chosen_meteo,np.round(Rpearson[0],6)),str(np.round(Rpearson[1],6)),str(np.round(roc_auc_score_value,6)),str(np.round(Rpearson[0]*roc_auc_score_value,6)) \n')

nb_impact = 0
for chosen_impact in ['Impact_fct'] : # ['Total_Deaths'] #['Impact_fct']
    nb_meteo = 0
    for chosen_meteo in tqdm(meteo_criteria) :
        ...
        if bins_dico_func[chosen_meteo] == 'linear' :
            Y,bins_edges=np.histogram(meteo_list,bins=np.logspace(start=min_val*0.95, stop=max_val*1.05, num=30)) #histogram of the E-OBS heatwaves distribution
        ...
        plt.grid()
        plt.xlabel(chosen_meteo,size=15)#+' '+units_dico[chosen_meteo])
        plt.ylabel('Frequency',size=15)
        plt.title('Heatwaves distribution over '+chosen_meteo+' criterion',size=15)# ('+criteria_dico[chosen_meteo]+')',y=1)
        ...
        plt.title('Impact of the extreme heatwaves ('+chosen_impact+')',y=0.8,size=15)
        plt.colorbar(CS1,cax=cax,orientation='horizontal')

        #plt.tight_layout()
        plt.savefig("D:/Ubuntu/PFE/JUICCE/Output/Van_der_Wiel_graphs/"+the_variable+"/Heatwaves_distrib_"+chosen_impact+"_"+chosen_meteo+".png")
        #plt.show()
        #break
        ...
        print('global_score :', np.round(Rpearson[0]*roc_auc_score_value,6))
        file_txt.write(chosen_meteo+'\t'+str(np.round(Rpearson[0],6))+'\t'+str(np.round(Rpearson[1],6))+
            '\t'+str(np.round(roc_auc_score_value,6))+'\t'+str(np.round(Rpearson[0]*roc_auc_score_value,6))+'\n')
